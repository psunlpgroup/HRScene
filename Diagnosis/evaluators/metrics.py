...
def default_whitebackground_metrics(responses: List[dict]) -> dict:
    eval_results = []

    for response in responses:
        prediction = clean_response(response["response"])
        score = min(sum(ans.lower() in prediction.lower() for ans in response["answer"]) / 3, 1) if prediction else 0
...
def default_complexgrid_metrics(responses: List[dict]) -> dict:
    eval_results = []

    for response in responses:
        id, row, col = response["id"].split("_")
